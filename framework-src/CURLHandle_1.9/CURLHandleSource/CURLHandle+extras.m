	...
	headers: (NSDictionary *) headers 
{
	...
	while (key = [keysEnum nextObject]) {
		...
		const char *filenameCString = NULL;
		const char *mimeTypeCString = NULL;
		...
		if ([value isKindOfClass: [NSData class]]) {
			...
			length = strlen(valueCString);
		} else if ([value isKindOfClass: [NSString class]]) {
			valueCString = [(NSString *) value UTF8String];
			length = strlen(valueCString);
		} else if ([value isKindOfClass: [NSDictionary class]]) {
			NSData *data = [value valueForKey:@"data"];
			valueCString = [data bytes];
			length = [data length];
			filenameCString = [[value valueForKey:@"filename"] UTF8String];
			if (!filenameCString) filenameCString = "curlhandle_filename_missing.dat";
			mimeTypeCString = [[value valueForKey:@"mimeType"] UTF8String];
			if (!mimeTypeCString) filenameCString = "application/octet-stream";
		}
		...
		if (customHeader) {
			...
			//fixme: the data buffers in the filename case don't stick around
			//so this probably only works with the synchronous usage
			
			// add the value to the form using the custom header
			if (filenameCString) {
				// with filename parameter
				curl_formadd(&post, &last,
							 CURLFORM_COPYNAME, [key UTF8String], 
...
//				NSLog(@"two %s", filenameCString);
			...
			if (filenameCString) {
				...
				curl_formadd(&post, &last,
							 CURLFORM_COPYNAME, [key UTF8String], 
							 CURLFORM_BUFFERPTR, valueCString,
